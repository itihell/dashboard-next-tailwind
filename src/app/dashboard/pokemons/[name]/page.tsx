...
// Generando metadata para la cabecera de la pagina
...
// Esto se ejeccuta en build time
export async function generateStaticParams() {
  const data: PokemonsResponse = await fetch(
    `https://pokeapi.co/api/v2/pokemon?limit=151`
  ).then((res) => res.json());

  const static151PoArray = data.results.map((pokemon) => {
    return {
      name: pokemon.name,
    };
  });

  //console.log("se cargo el array de pokemons");

  return static151PoArray.map(({ name }) => ({ name: name }));
}
